...
{
  //--body start--
  for(int t1 = 0; t1 < 1000000; t1 += 100000) {
    //--body start--
    for(int i1 = MAX(t1-1, 0), i1max = MAX(t1, 0), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      data->k13[(b1 + 1)] = 0.0;
    }
    for(int i1 = MAX(t1, 0), i1max = MIN(5, t1 + 100000 + 1), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      data->k13[(b1 + 1)] = 0.0;
    }
    //--body end--//--body start--
    for(int i1 = MAX(t1-1, 5), i1max = MAX(t1, 5), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      int32_t localVal_3 = 1000000.0;
      data->k13[(b1 + 1)] = 0.1 * (data->q_prev[(i1 + -1)] - data->q_prev[i1]);
    }
    for(int i1 = MAX(t1, 5), i1max = MIN(1000000, t1 + 100000 + 1), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      int32_t localVal_3 = 1000000.0;
      data->k13[(b1 + 1)] = 0.1 * (data->q_prev[(i1 + -1)] - data->q_prev[i1]);
    }
    //--body end--//--body start--
    ...
    for(int i1 = MAX(t1, 0), i1max = MIN(5, t1 + 100000 + 0), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      data->q[i1] = 1.0;
    }
    //--body end--//--body start--
    for(int i1 = MAX(t1-0, 5), i1max = MAX(t1, 5), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      ...
      double localVal_12 = data->k13[(b1 + 1)];
      data->q[i1] = (localVal_7 + (localVal_12 + 0.1 * ((data->q_prev[(i1 + -1)] + data->k13[(b1 + -1 + 1)]) - (localVal_7 + localVal_12))) / 2.0);
    }
    for(int i1 = MAX(t1, 5), i1max = MIN(1000000, t1 + 100000 + 0), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      int32_t localVal_3 = 1000000.0;
      double localVal_7 = data->q_prev[i1];
      double localVal_12 = data->k13[(b1 + 1)];
      data->q[i1] = (localVal_7 + (localVal_12 + 0.1 * ((data->q_prev[(i1 + -1)] + data->k13[(b1 + -1 + 1)]) - (localVal_7 + localVal_12))) / 2.0);
...
int formura_initialize(Formura0Navi* data){
  ...
  if((data->q_prev = (double *)malloc(sizeof(double)*1000000)) == 0)
    return 0;
  if((data->k13 = (double *)malloc(sizeof(double)*100002)) == 0)
    return 0;
...
void formura_setup(Formura0Navi* data, char* dumpdata)
{
  ...
  for(int t1 = 0; t1 < 1000000; t1 += 100000) {
    ...
    for(int i1 = MAX(t1, 0), i1max = MIN(1000000, t1 + 100000 + 0), b1 = i1 - t1; i1 < i1max; ++i1, ++b1) {
      data->q[i1] = (i1 < 20.0 ? 1.0 : 0.0);
    }
    //--body end--
  }
