...
main() {
  ...
  /*  k1.compute_at(step, y);
  k2.compute_at(step, y);
  k3.compute_at(step,y);*/
  step.vectorize(x,8);
#if COMPROOT
  k1.compute_root();
  k2.compute_root();
...
#endif
#if COMPROOT
  step.compile_to_lowered_stmt("stmt_halide100-comproot.html", {q}, Halide::HTML);
  step.compile_to_static_library("libh100-comproot", {q}, "step");
#else
  step.compile_to_lowered_stmt("stmt_halide100.html", {q}, Halide::HTML);
  step.compile_to_static_library("libh100", {q}, "step");
#endif
  return 0;
}
