...
#define K1(xx, yy) (DT / DX / DX * (Q(-1+xx, yy) + Q(xx, -1+yy) + Q(1+xx, yy) + Q(xx, 1+yy) - 4.0f*Q(xx, yy)))
#define K2(xx, yy) (DT / DX / DX * ((Q(-1+xx, yy) + (K1(-1+xx, yy)/2.0f)) + (Q(xx, -1+yy) + (K1(xx, -1+yy)/2.0f)) + (Q(1+xx, yy) + (K1(1+xx, yy)/2.0f)) + (Q(xx, 1+yy) + (K1(xx, 1+yy)/2.0f)) - 4.0f*(Q(xx, yy) + (K1(xx, yy)/2.0f))))
#define K3(xx, yy) (DT / DX / DX * ((Q(-1+xx, yy) + (K2(-1+xx, yy)/2.0f)) + (Q(xx, -1+yy) + (K2(xx, -1+yy)/2.0f)) + (Q(1+xx, yy) + (K2(1+xx, yy)/2.0f)) + (Q(xx, 1+yy) + (K2(xx, 1+yy)/2.0f)) - 4.0f*(Q(xx, yy) + (K2(xx, yy)/2.0f))))
...
kernel_step(const int x, const int y, real2d_t q, real2d_t qo) {
  if(5 <= x && 5 <= y && x < TOTAL_GRID_POINTS - 5 && y < TOTAL_GRID_POINTS - 5) {
    ...
    real_t k3 = K3(0, 0);
    real_t k4 = K4(0, 0);
    PSGridEmit(qo, qq + (k1/6.0f + k2/3.0f + k3/3.0f + k4/6.0f));
  } else {
    PSGridEmit(qo, 0.0f);
  }
}
...
int main(int argc, char ** argv) {
  ...
  if (strcmp(argv[1], "bench") == 0) {
    ...
    PSStencilRun(PSStencilMap(kernel_step, dom, q, qo), PSStencilMap(kernel_step, dom, qo, q), 5000);
