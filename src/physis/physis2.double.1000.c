...
kernel_k1(const int x, const int y, real2d_t q, real2d_t k1) {
  if(5 <= x && 5 <= y && x < TOTAL_GRID_POINTS - 5 && y < TOTAL_GRID_POINTS - 5) {
    PSGridEmit(k1, DT/DX/DX*(Q(-1,0) + Q(0, -1) + Q(1, 0) + Q(0, 1) - 4.0*Q(0, 0)));
  } else {
    PSGridEmit(k1, 0.0f);
  }
}
...
kernel_k2(const int x, const int y, real2d_t q, real2d_t k1, real2d_t k2) {
  if(5 <= x && 5 <= y && x < TOTAL_GRID_POINTS - 5 && y < TOTAL_GRID_POINTS - 5) {
    PSGridEmit(k2, DT / DX / DX * ((Q(-1, 0) + (K1(-1, 0)/2.0)) + (Q(0, -1) + (K1(0, -1)/2.0)) + (Q(1, 0) + (K1(1, 0)/2.0)) + (Q(0, 1) + (K1(0, 1)/2.0)) - 4.0*(Q(0, 0) + (K1(0, 0)/2.0))));
  } else {
    PSGridEmit(k2, 0.0);
  }
}
...
kernel_k3(const int x, const int y, real2d_t q, real2d_t k2, real2d_t k3) {
  if(5 <= x && 5 <= y && x < TOTAL_GRID_POINTS - 5 && y < TOTAL_GRID_POINTS - 5) {
    PSGridEmit(k3, DT / DX / DX * ((Q(-1, 0) + (K2(-1, 0)/2.0)) + (Q(0, -1) + (K2(0, -1)/2.0)) + (Q(1, 0) + (K2(1, 0)/2.0)) + (Q(0, 1) + (K2(0, 1)/2.0)) - 4.0*(Q(0, 0) + (K2(0, 0)/2.0))));
  } else {
    PSGridEmit(k3, 0.0);
  }
}
...
kernel_step(const int x, const int y, real2d_t q, real2d_t k1, real2d_t k2, real2d_t k3, real2d_t qo) {
  if(5 <= x && 5 <= y && x < TOTAL_GRID_POINTS - 5 && y < TOTAL_GRID_POINTS - 5) {
    ...
    real_t kk3 = K3(0, 0);
    real_t kk4 = K4(0, 0);
    PSGridEmit(qo, qq + (kk1/6.0 + kk2/3.0 + kk3/3.0 + kk4/6.0));
  } else {
    PSGridEmit(qo, 0.0);
  }
}
  ...
  if (strcmp(argv[1], "bench") == 0) {
    ...
    if(clock_gettime(CLOCK_MONOTONIC, &start)) return 0;
      PSStencilRun(PSStencilMap(kernel_k1, dom, q, k1),
		   ...
		   PSStencilMap(kernel_step, dom, qo, k1, k2, k3, q), 5000);
